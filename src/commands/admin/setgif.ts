...
async function setGifHandler(ctx: BotContext) {
  ...
  const newId = parseFileId(msg.reply_to_message.animation.file_id).id.toString();
  const updated = [...existing, newId];

  await db
    .insert(schema.groups)
    .values({ id: msg.chat.id, name: msg.chat.title, gifIds: updated })
    .onConflictDoUpdate({
      target: [schema.groups.id],
      set: { name: msg.chat.title, gifIds: updated },
    });
